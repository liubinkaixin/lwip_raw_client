...
__align(4) u8 SDIO_DATA_BUFFER[512];                    //缓冲区	
...
SD_Error SD_Init(void)
{
	...
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource8,GPIO_AF_SDIO);   //各个引脚一次映射到SDIO功能
...
SD_Error SD_PowerON(void)
{
 ...
 //初始化时的时钟不能大于400KHz 
	SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;	         //初始化频率
...
/*--------------------------------------------------------------------*/
/*函数名：设置SDIO总线宽度(MMC卡不支持4bit模式)                       */
/*参  数：wmode:位宽模式.0,1位数据宽度;1,4位数据宽度;2,8位数据宽度    */
/*返回值：0：无错误  其他：错误                                       */
/*--------------------------------------------------------------------*/
...
/*-------------------------------------------------*/
/*函数名：得到NumberOfBytes以2为底的指数.          */
/*参  数：NumberOfBytes:字节数.                    */
/*返回值：返回值:以2为底的指数值	               */
/*-------------------------------------------------*/	

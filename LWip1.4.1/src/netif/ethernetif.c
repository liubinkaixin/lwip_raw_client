...
extern ETH_DMADESCTypeDef  DMARxDscrTab[ETH_RXBUFNB], DMATxDscrTab[ETH_TXBUFNB];                    //发送和接收DMA描述符数组
extern uint8_t Rx_Buff[ETH_RXBUFNB][ETH_MAX_PACKET_SIZE], Tx_Buff[ETH_TXBUFNB][ETH_MAX_PACKET_SIZE];//发送和接收缓冲区
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  int l = 0;      //长度
	u8 *buffer;
//	printf("start Eth and ");
  buffer =  (u8 *)ETH_GetCurrentTxBuffer();//获取当前DMA内缓冲区指针，将要发送的数据，拷贝到缓冲区
...
//	int i;
  ...
  DMATxDescToSet->ControlBufferSize = (FrameLength & ETH_DMATxDesc_TBS1);//设置发送帧长度
	dma_led_cpl();
//	printf("start:\r\n");
//	for(i = 0;i < DMATxDescToSet->ControlBufferSize;i ++)
//	{
//		printf("%02X ",((u8 *)(DMATxDescToSet->Buffer1Addr))[i]);
//		if(0 == i%50)
//		{
//			if(i != 0)
//			{
